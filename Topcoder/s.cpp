...
int L;
void CountMiddle(int i, int use, int use1, int use2) {
	if (i == L) {
	...
	if (TestBit(use, i)) {
		CountMiddle(i + 1, use, use1, use2);
	} else {
		int nlen = L - 1;

		// Left Up
		if (i - 1 >= 0 && !TestBit(use1, i - 1)) {
			CountMiddle(i + 1, use | (1 << i), use1 | (1 << (i - 1)), use2);
		}
		// Right Up
		if (i < nlen && !TestBit(use1, i)) {
			CountMiddle(i + 1, use | (1 << i), use1 | (1 << i), use2);
		}

		// Left Down
		if (i - 1 >= 0 && !TestBit(use2, i - 1)) {
			CountMiddle(i + 1, use | (1 << i), use1, use2 | (1 << (i - 1)));
		}
		// Right Down
		if (i < nlen && !TestBit(use2, i)) {
			CountMiddle(i + 1, use | (1 << i), use1, use2 | (1 << i));
		}
		// Horizontal
		if (i + 1 < L && !TestBit(use, i + 1)) {
			CountMiddle(i + 2, use | (1 << i) | (1 << (i + 1)), use1, use2);
		}
	}
}
